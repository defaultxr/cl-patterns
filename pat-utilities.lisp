...
(defgeneric shallow-copy-instance (object &rest initargs &key &allow-other-keys)
...
;; modified version of the above. doesn't really work though.
(defgeneric copy-instance (object &rest initargs &key &allow-other-keys)
  ...
  (:method ((object standard-object) &rest initargs &key &allow-other-keys)
    (let* ((class (class-of object))
      ...
      (dolist (slot-name (mapcar #'sb-mop:slot-definition-name (sb-mop:class-slots class)))
        (when (slot-boundp object slot-name)
          (let ((sv (slot-value object slot-name)))
            (setf (slot-value copy slot-name)
                  (if (subtypep (class-of sv) 'standard-object)
                      (copy-instance sv)
                      sv)))))
      (apply #'reinitialize-instance copy initargs))))
