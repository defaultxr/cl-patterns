...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defclass clock ()
    ((beat :initform 0 :accessor beat :type beat :documentation "The number of beats that have elapsed since the creation of the clock.")
     (tempo :initarg :tempo :initform 2 :reader tempo :type interval :documentation "The tempo of the clock, in beats per second.")
     (latency :initarg :latency :initform 1/10 :type (real 0) :documentation "The default latency for events played on the clock; the number of seconds added onto the event's scheduled time, in order to allow the backend to process it without being \"late\".")
     (play-expired-events :initarg :play-expired-events :initform nil :type (member t nil :warn) :documentation "If T, always play events, even if their scheduled time has already passed. If NIL, silently skip these events. If :WARN, print a warning for each event skipped.")
     ...
     (caught-conditions :initform nil :type list :documentation "A list of conditions caught by the clock while processing tasks when its condition-handler is non-nil. Each entry in the list is a plist containing the the condition and any extra data generated by the function designated by the caught-conditions-extra-data-function slot.")
     ...
     (caught-conditions-extra-data-function :initarg :caught-conditions-extra-data-function :initform nil :accessor clock-caught-conditions-extra-data-function :type (or null function-designator) :documentation "A function of one argument (the clock) that generates extra data for the `caught-conditions' when a condition occurs during the clock's task processing.")
     (tasks :initform nil :type list :documentation "List of tasks that are running on the clock. Use `play', `stop', etc to play and stop patterns (the \"friendly\" way to add or remove them from the clock), or `clock-add' and `clock-remove' to manually remove them directly.")
     (tasks-lock :initform (bt:make-recursive-lock) :documentation "The lock on the tasks to make the clock thread-safe.")
     (timestamp-at-tempo :initform (local-time:now) :type local-time:timestamp :documentation "The local-time timestamp when the tempo was last changed.")
     (beat-at-tempo :initform 0 :type beat :documentation "The number of beats on the clock when the tempo was last changed."))
                      ...
                      (bt:with-lock-held ((slot-value clock 'caught-conditions-lock))
                        (let ((restart (if (member (clock-condition-handler clock) (list 'remove-task 'skip-event))
                          ...
                          (push (list* :condition e
                                       (when-let ((function (clock-caught-conditions-extra-data-function clock)))
