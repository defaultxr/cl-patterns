...
A library for conveniently(?) writing patterns to generate or process (a)musical sequences of mathematically (un)related compound(?) values in Lisp.
...
As far as I'm aware there isn't a huge community of people using this library to make music yet. However, if you need support or want to chat about it, the "official" room is on [[https://matrix.to/#/#cl-patterns:matrix.org][Matrix: cl-patterns:matrix.org]].
