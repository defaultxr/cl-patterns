...
pbind supports a set of special keys that, during the initialization of the pbind, run a function which may inject key/value pairs into the pbind's pattern-pairs, or alter it in another way. For example, a pbind that looks like this:
