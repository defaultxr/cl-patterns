...
* make pstreams compatible with sequence functions (at least in sbcl; not sure if it would work for other implementations); http://www.sbcl.org/manual/#Extensible-Sequences
* Renoise import/export
* pure data OSC (make sure it works)
