...
** look into [[https://github.com/byulparan/sc-extensions/blob/master/pc.lisp][pc.lisp from sc-extensions]]
...
* [[https://en.wikipedia.org/wiki/ABC_notation][ABC notation]] - work started in [[file:../src/abc.lisp][src/abc.lisp]]
* "after actions"
...
* maybe allow ~clock~'s ~condition-handler~ to be a function that selects the condition to run
this would, for example, allow a function to first try ~skip-event~, and then ~remove-task~ if the task is still generating errors
* ~unwind-protect~ on the ~clock-loop~ to restart it automatically if the user chooses the quit restart when an error occurs during the ~clock-loop~ ?
* rename ~instrument~ to something shorter and better
has to be generic enough to make sense for different types of values. for example, it can't be ~synth~ because the user might control something that isn't a synth. likewise for ~patch~. maybe ~target~ could work? but i think that would conflict with cl-collider's usage...
* something like [[https://github.com/cofinley/why-does-that-sound-good][why-does-that-sound-good]] ?
