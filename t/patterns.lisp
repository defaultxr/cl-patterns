...
(test peek
  ...
  (is (= 0
         (let ((pstr (as-pstream (pbind :dur 1/3 :foo (pseq (list 1 2 3) 1)))))
           (peek pstr)
           (beat pstr)))
      "beat should not count peeked outputs"))
...
(test beat-key
  ...
  (is-true (every-event-equal
           ...
           ":beat key is not overwritten when events are yielded from pstreams")
  (is-true (every-event-equal
            (list
             (event :beat 3 :x 0)
             (event :beat 5 :x 1))
  ...
  (is (equal ;; FIX: make sure this works for all filter patterns (parp, etc)
       ...
       (gete
        (next-upto-n
         ...
         12)
        :x))
      "pr's REPEATS parameter has access to *EVENT* generated by its source pattern"))
