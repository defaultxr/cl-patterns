...
(test t-pstream
  ...
  (is-false (t-pstream-p (as-pstream (pseq (list 1))))
            "t-pstream-p gives incorrect results for non-t-pstreams")
  (let ((obj :foo))
...
(test ipstream
  "Test ipstream"
  (let ((ipstr (as-pstream (ipstream (list (pbind :dur (pn 1 4))) t 1/5))))
    (is (event-equal (event :dur 1 :delta 1/5)
                     (next ipstr))
        "ipstream doesn't yield the first output correctly")
    (is (rest-p (next ipstr))
        "ipstream doesn't pad the time after events")
    (ipstream-insert ipstr (pbind :x (pseries) :dur (pn 1/2 3)))
    (let* ((ip-beat (beat ipstr))
           (lst (list (+ ip-beat 1/2) (+ ip-beat 1) 1 2 3)))
      (is (event-equal (event :x 0 :dur 1/2 :delta 1/5)
                       (next ipstr))
  ...
  (is (every-event-equal (list (event :x 1 :dur 1 :delta 1/5)
                               ...
                               (event :type :rest :delta 1/5)
                               (event :type :rest :delta 1/5))
                         (next-upto-n (ipstream (list (pbind :x 1 :dur (pseq (list 1) 1))) t 1/5) 6))
      "ipstream doesn't yield correct outputs when its subpatterns have ended")
  (is (every-event-equal (list (event :dur 1 :delta 1/5)
                               (event :type :rest :delta 1/5)
                               (event :type :rest :delta 1/10)
                               (event :dur 1/2 :delta 1/5)
                               (event :type :rest :delta 1/5)
                               (event :type :rest :delta 1/10))
                         (next-upto-n (ipstream (list (pbind :dur (pn 1 1))
                                                      (list 1/2 (pbind :dur (pn 1/2 1))))
                                                t 1/5)
                                      7))
      "ipstream doesn't yield correct outputs for multiple subpatterns with differing start points")
  (is (every-event-equal (list (event :type :rest :delta 1/5)
                               (event :type :rest :delta 2/15)
                               (event :dur 1 :delta 1/6)
                               (event :dur 1/2 :delta 1/5)
                               (event :type :rest :delta 1/5)
                               (event :type :rest :delta 1/10)
                               (event :type :rest :delta 2/15))
                         (next-upto-n (ipstream (list (list 1/3 (pbind :dur (pn 1 1)))
                                                      (list 1/2 (pbind :dur (pn 1/2 1))))
                                                t 1/5)
                                      8))
